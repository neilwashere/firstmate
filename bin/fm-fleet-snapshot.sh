...
crew_state_json() {  # <id> [<captured-meta>] [<captured-status>]
  ...
  local raw_file="$SNAPSHOT_TASK_DIR/$id.crew-state-raw"
  local detail_file="$SNAPSHOT_TASK_DIR/$id.crew-state-detail"
  raw=$(
  ...
  # A status line carries an unbounded note, so the crew-state detail and raw
  # line it is derived from can exceed the 128 KiB MAX_ARG_STRLEN Linux imposes
  # on ONE exec argument. Both ride a file for that reason; only the bounded
  # state and source words stay on argv.
  printf '%s' "$raw" > "$raw_file" || return 1
  printf '%s' "$detail" > "$detail_file" || return 1
  jq -n --rawfile raw "$raw_file" --arg state "$state" --arg source "$source" \
    --rawfile detail "$detail_file" \
...
status_event_json() {  # <observed-status-log> <contract-path> <transport-prefix>
  ...
  printf '%s' "$raw" > "$prefix.event-raw" || return 1
  printf '%s' "$note" > "$prefix.event-note" || return 1
  jq -n \
    --arg path "$path" \
    --rawfile raw "$prefix.event-raw" \
    --arg verb "$verb" \
    --rawfile note "$prefix.event-note" \
    --argjson age "$age" \
...
task_json_lines() {
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    # A per-task observation this loop cannot read degrades that ONE task's
    ...
    while IFS= read -r observation_line || [ -n "$observation_line" ]; do
      ...
      printf 'fm-fleet-snapshot: %s: endpoint observation unreadable; reporting endpoint unknown\n' \
        "$id" >&2
      endpoint_exists=null
      agent_alive=unknown
    }
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      terminal_contradiction=$(jq -r --rawfile note "$event_note_file" '
        any(.activities[]; .verdict == "contradicts" and .summary == $note)' "$reconciliation_file")
      ...
      jq -n \
        ...
        --argjson registered "$registered" --argjson event_age "$event_age" --argjson observed_age "$observed_age" --slurpfile activities "$activities_file" --slurpfile activity_scan "$activity_scan_file" \
...
CONTRIBUTION_INPUT_JSON_FILE="$JSON_TRANSPORT_DIR/contribution-input.json"
printf '%s\n' "$BACKLOG_JSON" > "$BACKLOG_JSON_FILE" \
  || { echo "fm-fleet-snapshot: temporary backlog file write failed" >&2; exit 1; }

contribution_input_json() {
  local tasks_file="$JSON_TRANSPORT_DIR/contribution-tasks.json" tasks_json
  tasks_json=$(contribution_tasks_json) \
    || { echo "fm-fleet-snapshot: contribution task read failed" >&2; return 1; }
  printf '%s\n' "$tasks_json" > "$tasks_file" \
    || { echo "fm-fleet-snapshot: contribution task staging failed" >&2; return 1; }
  jq -n --slurpfile backlog "$BACKLOG_JSON_FILE" --slurpfile tasks "$tasks_file" \
...
if [ "$OUTPUT_MODE" = contribution-input ]; then
  ...
  contribution_input_json > "$CONTRIBUTION_INPUT_JSON_FILE" \
    || { echo "fm-fleet-snapshot: contribution input staging failed" >&2; exit 1; }
  cat "$CONTRIBUTION_INPUT_JSON_FILE" \
    || { echo "fm-fleet-snapshot: contribution input read failed" >&2; exit 1; }
  exit 0
fi
...
FM_CONTRIBUTIONS_NOW="$SNAPSHOT_NOW" "$SCRIPT_DIR/fm-contributions.sh" snapshot \
  "$CONTRIBUTION_INPUT_JSON_FILE" > "$CONTRIBUTIONS_JSON_FILE" \

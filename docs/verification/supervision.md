...
| Pi | 0.86.1 (pi-subagents 0.70.1) | Extension listens for `subagent:async-started` / `subagent:async-complete` on `pi.events` | An interactive Pi started one async `delegate` child and ended its turn: `busy event=agent-start`, then `busy event=subagents-running` with the progress marker refreshed every 30s while the child ran, then `busy event=agent-start` when the completion woke the parent, then `idle event=agent-settled`. |
